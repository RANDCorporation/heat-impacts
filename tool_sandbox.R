...
box_fig <- data %>%
  filter(!is.na(HeatRisk)) %>%
  ggplot(aes(x = HeatRisk, y = diff, fill = HeatRisk), alpha = 0.8) +
  geom_jitter(aes(color=HeatRisk),width = 0.2, size=0.4, alpha= 0.5) +
  ...
  scale_color_manual(values = c("None" = "palegreen3",
                               "Minor" = "darkgoldenrod2",
                               "Moderate" = "darkorange2",
                               "Major" = "red1",
                               "Extreme" = "purple4")) +
